...
if add_sidebar == 'Aggregate Metrics':
    ...
    df_agg_diff_final = df_agg_diff.loc[:,
                            ['Video title', 'Publish_date', 'Views', 'Likes', 'Subscribers', 'Shares',
                             'Comments added', 'RPM(USD)', 'Average % viewed', 'Avg_duration_sec', 'Engagement_ratio',
                             'Views / sub gained']
                        ]

    df_agg_numeric_boolean = np.array((df_agg_diff_final.dtypes == 'float64') | (df_agg_diff_final.dtypes == 'int64'))
    df_agg_numeric_lst = df_agg_diff_final.iloc[:,df_agg_numeric_boolean].columns
    df_to_pct = {}
